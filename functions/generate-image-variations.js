// Netlify Function for generating image variations
...
async function mainHandler(event, context) {
  try {
    ...
    const numVariations = data.num_variations || 3;
    ...
    // Call Stability AI API for multiple variations
    ...
    for (let i = 0; i < responseData.artifacts.length; i++) {
      const artifact = responseData.artifacts[i];
      
      // Generate a unique ID for each variation
      const imageId = 'img_var_' + Math.random().toString(36).substring(2, 10);
    ...
    // Return success response with variations
    return {
      statusCode: 200,
      headers: {
        'Access-Control-Allow-Origin': '*',
